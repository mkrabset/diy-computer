...
public class DIYInstructionSet extends InstructionSet {
    ...
    protected List<Instruction> getInstructions() {
        ...
        instructions.addAll(genIns(regs, this::createLoadImmediate));
        instructions.addAll(genIns(regs, this::createLoadAbsolute));
        instructions.addAll(genIns(regs, this::createLoadIndirect));
        instructions.addAll(genIns(regs, this::createStoreAbsolute));
        instructions.addAll(genIns(regs, this::createAddImmediate));
        instructions.addAll(genIns(regs, this::createSubImmediate));
        instructions.addAll(genIns(regs, this::createCmpImmeditate));
        ...
        instructions.addAll(genIns(regs, r -> createAndOrXorImmediate(r, "AND", c.alu.andOpSignals)));
        instructions.addAll(genIns(regs, r -> createAndOrXorImmediate(r, "OR", c.alu.orOpSignals)));
        instructions.addAll(genIns(regs, r -> createAndOrXorImmediate(r, "XOR", c.alu.xorOpSignals)));
        ...
        instructions.addAll(genIns2(regPairs, rp -> createLoadAbsoluteIndexed(rp.r1, rp.r2)));
        instructions.addAll(genIns2(regPairs, rp -> createLoadIndexedIndirect(rp.r1, rp.r2)));
        instructions.addAll(genIns2(regPairs, rp -> createLoadIndirectIndexed(rp.r1, rp.r2)));

        instructions.addAll(genIns2(regPairs, rp -> createStoreAbsoluteIndexed(rp.r1, rp.r2)));
        instructions.addAll(genIns2(regPairs, rp -> createStoreIndexedIndirect(rp.r1, rp.r2)));
        ...
        instructions.add(createRolAbsolute());
        instructions.add(createRorAbsolute());
        instructions.add(createNotAbsolute());
        instructions.add(createIncDecAbsolute("INC", true));
        instructions.add(createIncDecAbsolute("DEC", false));
        ...
        // Unconditional jumps
        instructions.add(createBranchAbsolute("JMP", PC.JumpCondition.UNCOND));
        ...
        instructions.addAll(
                Arrays.stream(PC.JumpCondition.values())
                        ...
                        .map(cond -> createBranchAbsolute("B" + cond.name(), cond))
                        .collect(Collectors.toList())
        );

        // Subroutine jump/return
        instructions.add(createJsrAbsoluteOrIndirect(false));
        instructions.add(createJsrAbsoluteOrIndirect(true));
        instructions.add(createRts());

        // Dummy fill the rest
        for (int i = instructions.size(); i < 254; i++) {
            instructions.add(new Instruction(c, "dummy", "dummy", "dummy", "na"));
        }
    ...
    private Instruction createCmpRegs(Register r1, Register r2) {
        Instruction i = new Instruction(c, "CMP" + r1.name, "CMP" + r1.name + "," + r2.name, "flags := " + "cmp(" + r1.name + "," + r2.name + ")", "na");
    ...
    private Instruction createTransferRegs(Register r1, Register r2) {
        Instruction i = new Instruction(c, "T" + r1.name + r2.name, "T" + r1.name + r2.name, r2.name + " := " + r1.name, "na");
        addStep(i, r1.busWrite(), r2.busRead());
        return i;
    }

    private Instruction createNop() {
        Instruction i = new Instruction(c, "NOP", "NOP", "No operation", "na");
        return i;
    }

    private Instruction createAddAbsolute(Register r) {
        Instruction i = new Instruction(c, "ADD" + r.name, "ADD" + r.name + " \\$(....)", r.name + " := " + r.name + " + ram(arg)", "absolute");
        argsToMar(i);
    ...
    private Instruction createSubAbsolute(Register r) {
        Instruction i = new Instruction(c, "SUB" + r.name, "SUB" + r.name + " \\$(....)", r.name + " := " + r.name + " - ram(arg)", "absolute");
        argsToMar(i);
        addStep(i, r.busWrite(), ALU_A_IN);
        addStep(i, RAM_OUT, ALU_B_IN);
    ...
    private Instruction createCmpAbsolute(Register r) {
        Instruction i = new Instruction(c, "CMP" + r.name, "CMP" + r.name + " \\$(....)", "flags := " + "cmp(" + r.name + ",ram(arg))", "absolute");
        argsToMar(i);
        addStep(i, r.busWrite(), ALU_A_IN);
    ...
    private Instruction createAndOrXorImmediate(Register r, String opName, Signal[] aluOpSignals) {
        Instruction i = new Instruction(c, opName + r.name, opName + r.name + " #(..)", r.name + " := " + r.name + " " + opName + " arg", "immediate");
        addStep(i, r.busWrite(), ALU_A_IN);
    ...
    private Instruction createAndOrXorAbsolute(Register r, String opName, Signal[] aluOpSignals) {
        Instruction i = new Instruction(c, opName + r.name, opName + r.name + " \\$(....)", r.name + " := " + r.name + " " + opName + " ram(arg)", "absolute");
        argsToMar(i);
        addStep(i, r.busWrite(), ALU_A_IN);
        addStep(i, RAM_OUT, ALU_B_IN);
        addStep(i, ALU_OUT, r.busRead(), aluOpSignals, c.alu.updateFlagsSignal);
    ...
    private Instruction createRol(Register r) {
        Instruction i = new Instruction(c, "ROL" + r.name, "ROL" + r.name, r.name + " := ROL(" + r.name + ")", "na");
        addStep(i, r.busWrite(), ALU_A_IN);
    ...
    private Instruction createRor(Register r) {
        Instruction i = new Instruction(c, "ROR" + r.name, "ROR" + r.name, r.name + " := ROR(" + r.name + ")", "na");
        addStep(i, r.busWrite(), ALU_B_IN);
    ...
    private Instruction createInc(Register r) {
        Instruction i = new Instruction(c, "INC" + r.name, "INC " + r.name, r.name + " := " + r.name + " + 1 ", "na");
    ...
    private Instruction createDec(Register r) {
        Instruction i = new Instruction(c, "DEC" + r.name, "DEC " + r.name, r.name + " := " + r.name + " - 1 ", "na");
        addStep(i, BusWriter.ZEROS, BusReader.ALU_B_IN);
    ...
    private Instruction createOp2regs(String opName, Signal[] opSignals, Register r, Register r2, boolean subMode) {
        Instruction i = new Instruction(c, opName + r.name + r2.name, opName + r.name + " " + r2.name, r.name + " := " + r.name + " " + opName + " " + r2.name, "na");
        addStep(i, r.busWrite(), ALU_A_IN);
    ...
    private Instruction createRolAbsolute() {
        Instruction i = new Instruction(c, "ROL", "ROL \\$(....)", "ram(arg):=ROL(ram(arg))", "absolute");
        argsToMar(i);
    ...
    private Instruction createRorAbsolute() {
        Instruction i = new Instruction(c, "ROR", "ROR \\$(....)", "ram(arg):=ROR(ram(arg))", "absolute");
        argsToMar(i);
        addStep(i, RAM_OUT, ALU_B_IN);
    ...
    private Instruction createNotAbsolute() {
        Instruction i = new Instruction(c, "NOT", "NOT \\$(....)", "ram(arg):=NOT(ram(arg))", "absolute");
        argsToMar(i);
        addStep(i, ZEROS, ALU_A_IN);
    ...
    private Instruction createIncDecAbsolute(String opName, boolean inc) {
        Instruction i = new Instruction(c, opName, opName + " \\$(....)", "ram(arg):=" + opName + "(ram(arg))", "absolute");
        argsToMar(i);
        addStep(i, BusWriter.ZEROS, BusReader.ALU_B_IN);
        if (inc) {
    ...
    private Instruction createSec() {
        Instruction i = new Instruction(c, "SEC", "SEC", "flags.c=true", "na");
        addStep(i, c.alu.setCarrySignal);
        return i;
    }

    private Instruction createClc() {
        Instruction i = new Instruction(c, "CLC", "CLC", "flags.c=false", "na");
        addStep(i, c.alu.clearCarrySignal);
        return i;
    }

    private Instruction createBranchAbsolute(String opName, PC.JumpCondition jumpCond) {
        Instruction i = new Instruction(c, opName, opName + " \\$(....)", "if (" + jumpCond.descr + ") pc=arg", "absolute");
    ...
    private Instruction createJumpIndirect() {
        Instruction i = new Instruction(c, "JMP", "JMP \\(\\$(....)\\)", "pc=ram(arg)", "indirect");
        argsToMar(i);
    ...
    private Instruction createJsrAbsoluteOrIndirect(boolean indirect) {
        Instruction i = indirect
                ? new Instruction(c, "JSR", "JSR \\(\\$(....)\\)", "if (cond) pc=ram($address), push returnaddress", "indirect")
                : new Instruction(c, "JSR", "JSR \\$(....)", "if (cond) pc=address, push returnaddress", "absolute");
        if (indirect) {
            argsToMar(i);
        }
    ...
    private Instruction createRts() {
        Instruction i = new Instruction(c, "RTS", "RTS", "pc=pop()", "na");
    ...
    private Instruction createPush(Register r) {
        Instruction i = new Instruction(c, "PUSH" + r.name, "PUSH " + r.name, "push(" + r.name + ")", "na");
    ...
    private Instruction createPushXYZ() {
        Instruction i = new Instruction(c, "PUSHXYZ", "PUSHXYZ", "push([x,y,z])", "na");
        addStep(i, c.sp.cntSignal, c.sp.dirDownSignal); // decrease sp
    ...
    private Instruction createPop(Register r) {
        Instruction i = new Instruction(c, "POP" + r.name, "POP " + r.name, r.name + "=pop()", "na");
    ...
    private Instruction createPopZYX() {
        Instruction i = new Instruction(c, "POPZYX", "POPZYX", "[y,z,x] = [pop(),pop(),pop()]", "na");
    ...
    private Instruction createOut(Register r, int port, BusReader in) {
        Instruction i = new Instruction(c, "OUT" + port, "OUT" + port + " " + r.name, "out#" + port + " := " + r.name, "na");
        addStep(i, r.busWrite(), in);
        return i;
    }

    private Instruction createIn(Register r, int port, BusWriter out) {
        Instruction i = new Instruction(c, "IN" + port, "IN" + port + " " + r.name, r.name + " := in#" + port, "na");
        addStep(i, out, r.busRead());
        return i;
    }

    private Instruction createHalt() {
        Instruction i = new Instruction(c, "HLT", "HLT", "halt", "na");
